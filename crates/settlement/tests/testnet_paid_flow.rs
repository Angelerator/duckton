...
use p2p_settlement::ton::{
    build_escrow_terms, escrow_code_from_boc_base64, GlobalParamsClient, ToncenterRpc,
    TonSettlement,
};
...
fn env() -> Option<Env> {
    ...
    let mnemonic = std::env::var("TON_TESTNET_MNEMONIC").ok().filter(|s| !s.trim().is_empty())?;
    ...
    Some(Env { rpc: rpc.trim_end_matches('/').to_string(), api_key, mnemonic, gp_addr })
}
...
fn account_state(env: &Env, raw_addr: &str) -> (String, i128) {
    let raw = curl(env, &[format!("{}/getAddressInformation?address={raw_addr}", env.rpc)]);
    let v: serde_json::Value = serde_json::from_str(&raw).unwrap_or_default();
    let state = v["result"]["state"].as_str().unwrap_or("unknown").to_string();
...
fn read_seqno(env: &Env, wallet_raw: &str) -> u32 {
    ...
    let raw = curl(
        env,
        &["-H".into(), "Content-Type: application/json".into(), "-d".into(), body, format!("{}/runGetMethod", env.rpc)],
    );
    let v: serde_json::Value = serde_json::from_str(&raw).unwrap_or_default();
    let s = v["result"]["stack"][0][1].as_str().unwrap_or("0x0").trim().to_string();
...
fn wait_for_seqno(env: &Env, wallet_raw: &str, want: u32, label: &str) -> u32 {
    for attempt in 0..36 {
        ...
        if s >= want {
            println!("  [{label}] wallet seqno reached {s} after {}s", attempt * 5);
            return s;
        }
...
fn wait_for_state(env: &Env, raw_addr: &str, want_state: &str, label: &str) -> bool {
    for attempt in 0..36 {
        ...
        if state == want_state {
            println!("  [{label}] account state = {state} (balance {bal}) after {}s", attempt * 5);
            return true;
        }
        sleep(Duration::from_secs(5));
    }
    let (state, _) = account_state(env, raw_addr);
    println!("  [{label}] WARNING: account state still {state} (wanted {want_state}) after timeout");
    false
}
...
fn load_escrow_code() -> p2p_settlement::cell::Cell {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../ton/build/JobEscrow.json");
...
fn rust_driven_paid_flow_open_settle_is_accepted_on_chain() {
    ...
    println!("wallet state = {state}, start balance = {start_bal} nanoton (~{} TON)", start_bal as f64 / 1e9);
    ...
    assert!(start_bal > need, "wallet must have gas + the escrow bid + buffer (need >{need})");
    ...
    let gp_rpc = ToncenterRpc::new(&env.rpc, Some(env.api_key.clone()), "testnet", &env.mnemonic)
        .expect("params-client RPC builds");
    let params_client = GlobalParamsClient::new(gp_rpc, gp_addr);
    let synced_version = params_client.params_version().expect("read live params_version");
    ...
    let settlement = TonSettlement::with_escrow_code(
        rpc,
        load_escrow_code(),
        build_escrow_terms(&wallet, &[0u8; 32], 0),
        wallet,
    )
    ...
    assert_eq!(after_open, before_open + 1, "wallet seqno must increment by 1 (deploy external accepted)");
    ...
    assert_eq!(onchain_pv as u32, synced_version, "on-chain escrow params_version must equal the synced version");
    ...
    let onchain_hash_hex = get_hex(&env, &escrow_raw, "get_expected_hash").expect("get_expected_hash");
    let want_hash_hex = hex::encode(result_hash);
    println!("  on-chain expected_hash = 0x{onchain_hash_hex}; locked = 0x{want_hash_hex}");
    assert_eq!(onchain_hash_hex, want_hash_hex, "on-chain HTLC lock must equal the quorum result hash");

    // === 3) SETTLE: release the escrow (winner == our wallet → funds return). ===
    let outcome = SettlementOutcome {
        result_hash,
        winner: Payout { to: wallet, amount: WINNER_AMOUNT },
        participants: vec![],
        platform_fee: 0,
    };
    ...
    if let Some((settled, pv)) = read_escrow_state(&env, &escrow_raw) {
        ...
        assert!(settled, "escrow must be settled after the EscrowSettle was accepted");
        assert_eq!(pv, synced_version, "settled escrow still binds the synced params version");
    }

    let (_, end_bal) = account_state(&env, &wallet_raw);
    println!("end balance = {end_bal} nanoton (~{} TON)", end_bal as f64 / 1e9);
    println!(
