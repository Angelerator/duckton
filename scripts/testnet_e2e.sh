...
#
# NOTE: crates/config's [economics] currently has NO per-contract address field,
# so the deployed StakeVault / JobEscrow / RecordAnchor addresses are surfaced to
# the harness + the ton-live Rust test via $NET.env above (TON_TESTNET_*_ADDR).
# Wiring an [economics.contracts] block into crates/config is a follow-up that
# must edit that crate; the proposed shape is shown (commented) below.
[economics]
enabled         = true
settlement      = "onchain"
...
# Proposed (not yet read by crates/config):
# [economics.contracts]
