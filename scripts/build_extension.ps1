#!/usr/bin/env pwsh
# Build the loadable DuckDB extension and append the metadata footer so it can be
# LOADed by the duckdb CLI on Windows. Mirrors scripts/build_extension.sh (the
# Python + duckdb calls are OS-agnostic; this just uses Windows paths / the .dll).
#
...
$dylib = $null
foreach ($cand in @(
    "target/$profileDir/duckdb_p2p.dll",
    "target/$profileDir/libduckdb_p2p.dylib",
    "target/$profileDir/libduckdb_p2p.so")) {
...
Write-Host '==> wrote dist/duckdb_p2p.duckdb_extension'
